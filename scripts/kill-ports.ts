 ...
 * Listeners only — same rule as the former `scripts/demo.ts` freePorts, so
...
const listenerPids = (ports: number[]): number[] => {
 ...
 */
export const freePorts = (ports: number[]): { ports: number[], pids: number[] } => {
...
export type KillCommand = {name: string, aliases?: string[], help: string, run: () => void}

const runInteractive = () => {
  const commands: KillCommand[] = [
    {
      name: 'status',
      aliases: ['s', 'refresh'],
