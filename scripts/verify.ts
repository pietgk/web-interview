...
import { ROOT, STAGES, findStage, findStep } from './stages.ts'
...
const runInvocation = ({
  ...
  env,
  tolerateOffline,
}: Invocation): Promise<{ ok: boolean; offline: boolean; output: string }> =>
  new Promise((settle) => {
...
const coverageHeadline = async () => {
  try {
    ...
    const pct = ({ covered, total }: { covered: number; total: number }) => total === 0 ? 100 : (covered * 100 / total).toFixed(2)
