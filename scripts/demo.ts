...
const stopBackend = async () => {
  ...
  const child = (backend as ChildProcess)
  child.kill('SIGTERM')
...
const shutdown = async (exitCode = 0) => {
  ...
  for (const child of [preview, backend]) {
    if (isAlive(child)) (child as ChildProcess).kill('SIGTERM')
  }
  await Promise.all(
    [preview, backend].filter(isAlive).map((child) =>
      waitForExit((child as ChildProcess), 3_000)
    )
  )
  process.exit(exitCode)
}

export type Command = {name: string, aliases?: string[], help: string, run: () => unknown}
