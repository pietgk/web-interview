...
/** @typedef {Record<string, any>} RuleOptions */
...
/** @param {AstNode | null | undefined} node */
const getBindingName = (node) => node?.type === 'Identifier' ? node.name : null

/** @param {string} name */
const normaliseName = (name) => name.replaceAll('_', '').toLowerCase()
...
/** @param {AstNode} node */
const expressionMayBelongToBinding = (node) =>
...
/** @param {AstNode} node */
const numericExpressionRoot = (node) => {
  let current = node
  while (
...
/** @param {AstNode} node */
const enclosingCallArgument = (node) => {
  let current = node
...
/** @param {AstNode} matcherCall */
const expectSubject = (matcherCall) => {
