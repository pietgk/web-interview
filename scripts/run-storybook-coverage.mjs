...
import { createCoverageStabilitySnapshot } from './coverage-producers.mjs'
...
try {
  ...
  if (stabilityMode) {
    ...
    const firstDigest = firstSnapshot.digest
    const mismatch = snapshots.findIndex(({ digest }) => digest !== firstDigest)
    if (mismatch !== -1) {
      throw new Error(`Storybook controller coverage changed on collection ${mismatch + 1}`)
    }
    const { stdout: revision } = await execFileAsync('git', ['rev-parse', 'HEAD'], { cwd: ROOT })
