...
//
// `vitest.storybook.config.ts` is not a name it scans for, so before this file
...
//
// It is deliberately NOT what the root process runs. Root `vitest.config.ts`
// points straight at `vitest.logic.config.ts`, because pulling the browser
// project into the root process is what ADR 006 records as stalling.
export default defineConfig({
  test: {
    projects: ['./vitest.logic.config.ts', './vitest.storybook.config.ts'],
  },
})
