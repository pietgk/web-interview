...
import { createFakeDatomServer } from '../testing/fakeDatomServer.ts'
...
const clientFor = (
  server: ReturnType<typeof createFakeDatomServer>,
  overrides: Partial<Parameters<typeof createTodoClient>[0]> = {},
) =>
  createTodoClient({
    apiBase: '',
...
describe('createTodoClient', () => {
  ...
  it('rehydrates after rejection without losing or replaying a later write', async () => {
    ...
    const statuses: TodoClientStatus[] = []
  ...
  it('treats an invalid successful response as a browser failure and rehydrates', async () => {
    ...
    const commands = createTodoListCommands(client)
    let todoDuringRehydration: string | null | undefined
  ...
  it('marks the connection failed when the EventSource closes for good', async () => {
    ...
    const client = createTodoClient({
      EventSourceImpl: ClosedEventSource as unknown as typeof EventSource,
      fetchImpl: async () => {
  ...
  it('ignores a clock event whose serverTime is not a number', async () => {
    let clockListeners: Array<(event: unknown) => void> = []
    ...
    const client = createTodoClient({
      EventSourceImpl: ClockEventSource as unknown as typeof EventSource,
      fetchImpl: async () => {
        throw new Error('unexpected fetch')
      },
    })
