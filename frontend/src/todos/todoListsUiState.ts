 ...
 */
export type TodoListsUiState =
  | {mode: 'loading', activeListId: null}
  | {mode: 'browsing', activeListId: string | null}
  | {mode: 'drafting', activeListId: string, reservedListId: string}
  | {mode: 'confirmingDelete', activeListId: string | null, targetListId: string}

export type TodoListsUiEvent =
  | {type: 'DRAFT_STARTED', reservedListId: string}
  | {type: 'LIST_SELECTED', listId: string}
  | {type: 'LIST_MATERIALIZED', listId: string}
  | {type: 'DRAFT_ESCAPED'}
  | {type: 'DELETE_REQUESTED', targetListId: string}
  | {type: 'DELETE_CANCELLED'}
  | {type: 'DELETE_CONFIRMED', nextListId: string | null}
  | {type: 'LIST_HYDRATED', listId: string | null}
  | {type: 'UI_RESET'}

export const initialTodoListsUiState: TodoListsUiState = { mode: 'loading', activeListId: null }

export const todoListsUiReducer = (state: TodoListsUiState, event: TodoListsUiEvent): TodoListsUiState => {
  switch (event.type) {
    case 'DRAFT_STARTED':
